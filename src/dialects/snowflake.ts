...
export const snowflakeOptions: DialectOptions = {
  name: 'snowflake',
  hasSchema: true,
  /**
  ...
  isSerialKey: (record: FKRow) => {
    return (
      _.isObject(record) && snowflakeOptions.isPrimaryKey(record) && (_.has(record, 'is_identity') && record.is_identity)
    );
  },
  /**
   * Override Sequelize's method for showing all tables to allow schema support.
   * See sequelize/lib/dialects/snowflake/query-generator.js:showTablesQuery()
  ...
  /** Sequelize "describeTable" doesn't include precision and scale in snowflake */

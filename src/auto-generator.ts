  ...
  private addField(table: string, field: string): string {
    ...
    fieldAttrs.forEach(attr => {
      ...
      if (attr === "foreignKey") {
        ...
        if (this.dialect.name === ("mssql" || "db2") && (["(NULL)", "NULL"].includes(defaultVal) || typeof defaultVal === "undefined")) {
          defaultVal = null as any; // Override default NULL in MS SQL and db2 to javascript null
        }
